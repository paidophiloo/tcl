...
function clubCode(team){return String(team?.code||team?.tla||team?.id||'').toUpperCase().replace(/^CLUB-/,'')}
function applyOfficialLogos(root=document){
  const snapshot=activeEngine()?.getSnapshot?.();
  if(!snapshot?.teams?.length)return;
  const urls=snapshot.teams.map(t=>OFFICIAL_CLUB_LOGO_MANIFEST[clubCode(t.team)]?.logoUrl||null);
  root.querySelectorAll('.tlm3-team img').forEach((img,index)=>{const url=urls[index];if(url){img.src=url;img.style.visibility='visible';img.onerror=()=>{img.style.visibility='hidden'}}});
  root.querySelectorAll('.tlm3-full-score img').forEach((img,index)=>{const url=urls[index];if(url){img.src=url;img.style.visibility='visible';img.onerror=()=>{img.style.visibility='hidden'}}});
}
