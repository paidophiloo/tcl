...
assert.match(articleUi, /claim\.kind === 'manager-change'/, 'article reader must render verified manager and board-pressure claims');
assert.match(articleUi, /claim\.kind === 'contract'/, 'article reader must render verified contract claims');
assert.match(articleUi, /claim\.action === 'manager\.under-pressure'/, 'article reader must distinguish factual board pressure from manager changes');
assert.match(articleUi, /confiança da diretoria em nível/, 'board-pressure proof must show persisted confidence rather than a vague crisis headline');
assert.match(articleUi, /PPG reais vs/, 'board-pressure proof must expose real versus expected points when available');
