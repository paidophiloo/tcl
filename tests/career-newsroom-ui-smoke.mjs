...
assert.match(ui, /WORLD_PLAYER_BY_ID\.get\(playerId\)/, 'player-based story arcs must resolve the actual player name');
assert.match(ui, /'club\.manager-pressure': 'DIRETORIA'/, 'board pressure must have an explicit story-arc presentation');
assert.match(ui, /'club\.manager-vacancy': 'COMANDO TÉCNICO'/, 'manager vacancy must have an explicit story-arc presentation');
assert.match(ui, /'club\.manager-transition': 'NOVO CICLO'/, 'new manager transition must have an explicit story-arc presentation');
assert.match(ui, /'player\.contract-risk': 'CONTRATO'/, 'unresolved renewal risk must have an explicit story-arc presentation');
assert.match(ui, /'player\.bosman-agreement': 'PRÉ-CONTRATO'/, 'Bosman agreement must have an explicit story-arc presentation');
assert.match(ui, /confiança.*nível crítico|confiança.*sob pressão/, 'manager-pressure arc must expose persisted board confidence');
assert.match(ui, /renovação sem acordo/, 'contract-risk arc must present the unresolved factual state');
